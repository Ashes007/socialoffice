  ...
  <section class="content"> @include('admin.includes.messages')
    ...
    @foreach($event_posts as $post)
      <div class="timelineBlock" >
        ...
        <?php        
          if(file_exists( public_path('uploads/posts/thumbnails/'.$post->image) )&& ($post->image!='' || $post->profile_photo!=NULL)) {?>
        ...
        <div class="col-sm-7">
        <p><a  class='cntlikecls' href="{{URL::Route('eventlikelist').'/'.$post->id.'/'.encrypt($event_id)}}"><span id="likecnt_id_{{$post->id}}" class="likecls">{{count($post->likes)}} {{ __('common.likes')}} </span></a>- <a href="javascript:void(0);" class="user-com" data-target="{{$post->id}}" id="cmncnt_id_{{$post->id}}">{{count($post->comments)}} {{ __('common.comments')}}</a></p>
    ...
    @endforeach 
    @php echo $event_posts->links(); @endphp   
    @else
      <div class="timelineBlock" >
      <div class="time-postedBy">
...
$( document ).on( "click", ".deletepost", function(event) {    
  ...
  $.confirm({
    ...
    buttons: {
      confirm: function () { 
      window.location.href ="{{URL::Route('delete_posts')}}"+'/'+post_id+ '/'+event_id+'/event';
      },
      cancel: function () {
...
$( document ).on( "click", ".deletecomment", function(event) {    
  ...
  $.confirm({
    ...
    buttons: {
      confirm: function () { 
      window.location.href ="{{URL::Route('delete_comments')}}"+'/'+comment_id+ '/'+event_id+'/event';
      },
      cancel: function () {

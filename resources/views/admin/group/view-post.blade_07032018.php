  ...
  <section class="content"> @include('admin.includes.messages')
    ...
    @foreach($group_posts as $post)
      <div class="timelineBlock" >
        ...
        <span style="float: right;"><a href="javascript::void(0);" alt="{{$post->id}}" data-toggle="tooltip" class="deletepost" title="Delete Post"><i class="fa fa-trash"></i></a></span>
        </div>
        ...
        @endif     </div>
        <h2>{{ $comment->user->display_name }} <span style="float: right;"><a href="javascript::void(0);" alt="{{$comment->id}}" data-toggle="tooltip" class="deletecomment" title="Delete Comment"><i class="fa fa-trash"></i></a></span>&nbsp;<span>{{ \DateTime::createFromFormat('Y-m-d H:i:s', $comment->created_at)->format('dS M Y h:i A') }}&nbsp;&nbsp;</span></h2>
        <p>{{$comment->body}}</p>

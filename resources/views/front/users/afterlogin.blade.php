...
<div id="wrap">
    <div class="cube">
      <section class="page active face front" id="home">
          ...
          <span  class="animated zoomIn">{{ trans('usersAfterLogin.Registration') }}</span>
      ...
      <div class="reg-body">
            <p>{{ trans('usersAfterLogin.You_are') }} <input type="text" value="{{ $userInfo->display_name }}" readonly="readonly" placeholder="Full Name" data-autosize-input='{ "space": 20 }' />, {{ trans('usersAfterLogin.working_in_our') }} <input type="text" class="in2" value="{{ $userInfo->department->name }}" readonly="readonly" placeholder="Department" data-autosize-input='{ "space": 20 }' /> {{ trans('usersAfterLogin.Department_as') }}<input type="text" class="in3" value="{{ $userInfo->designation->name }}" readonly="readonly" placeholder="Title" data-autosize-input='{ "space": 20 }' />.</p>
              ...
              <p>{{ trans('usersAfterLogin.You_have_joined') }}<input type="text" class="in3" value="{{ $userInfo->company->name }}" readonly="readonly" placeholder="Company" data-autosize-input='{ "space": 20 }' /> {{ trans('usersAfterLogin.on') }} <input type="text" name="date_of_joining" id="date_of_joining"  placeholder="DD/MM/YYYY" data-date-format="dd/mm/yyyy" value="{{ ($userInfo->date_of_joining != '') ? \DateTime::createFromFormat('Y-m-d', $userInfo->date_of_joining)->format('d/m/Y') : '' }}" readonly="readonly" class="date" />.</p>

              <p>{{ trans('usersAfterLogin.Can_you_tell_me_your_date_of_birth') }}<input type='text' placeholder="DD/MM" name="birthday" id="birthday" readonly="readonly"  data-date-format="dd/mm" class="date1" value="{{ ($userInfo->date_of_joining != '') ? \DateTime::createFromFormat('Y-m-d', $userInfo->date_of_birth)->format('d/m') : '' }}" data-autosize-input='{ "space": 20 }'/>.</p>
            ...
            <header>
              ...
              <a class="skip_first" href="{{ URL::Route('home') }}">{{ trans('usersAfterLogin.Skip') }}</a></li>
            </header>
          </div>
        </div>
            </div>
     ...
     {!! Form::hidden('page_name','after-login') !!}
      ...
      <div class="reg-body reg-body2">
          ...
          <span id="charNum"></span>
              ...
              <p>{{ trans('usersAfterLogin.Please_upload_an_image_of_yourself') }}</p>
                <div class="croppings">
                  ...
                  <label for="open" style="width: 90%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; display: block;">
                    ...
                    {{ trans('usersAfterLogin.Browse_your_files') }}
                  </label>
              ...
              <input type="submit" value="{{ trans('usersAfterLogin.submit') }}"/> <span>{{ trans('usersAfterLogin.or') }}</span> <a href="{{ URL::Route('home') }}">{{ trans('usersAfterLogin.Skip') }}</a>
              </div>
          </div>

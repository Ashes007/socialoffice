...
<?php if(count($feed)): ?>
<?php if(eventInviteCheck($feed->id,$loggedin_user) > 0): ?>
<!-- ------------ event - -- -->
<div class="timelineBlock eventblock">
  ...
  <a href="<?php echo e(route('event_details', encrypt($feed->id))); ?>" ><img src="<?php echo e(asset('timthumb.php')); ?>?src=<?php echo e(asset('uploads/event_images/original/'.$feed->eventImage[0]->image_name)); ?>&w=586&h=175&q=100" alt=""></a>
  <?php else: ?>
  <a href="<?php echo e(route('event_details', encrypt($feed->id))); ?>" ><img src="<?php echo e(asset('frontend/images/no-image-event-home.jpg')); ?>" alt=""></a>
  <?php endif; ?>
  </div>
  ...
  <?php if($feed->getStatus($feed->id) == 1 || $feed->getStatus($feed->id) == 4): ?>
  <span> - <?php echo e(trans('common.attending')); ?></span> 
  <?php elseif($feed->getStatus($feed->id) == 2 || $feed->getStatus($feed->id) == 6): ?>
  ...
  <h5 class="location"><i class="fa fa-map-marker"></i><?php echo e(str_limit($feed->location,70,'...')); ?></h5>
  <?php endif; ?>
  </div>
  ...
  <?php elseif($feed->getStatus($feed->id) == 6): ?>
      <div class="attend_status_button not"><?php echo e(trans('buttonTxt.not_attended')); ?></div>
  <?php endif; ?>
  <?php else: ?>

  <?php if($feed->event_end_date > $today): ?>

  <?php if($feed->getStatus($feed->id) == 1 || $feed->getStatus($feed->id) == 4): ?>
    <a href="javascript:void(0);" class="not_go event_response .canBtn" data-eventId = "<?php echo e($feed->id); ?>" data-status="6"><?php echo e(trans('buttonTxt.not_attend')); ?></a>
  <?php elseif($feed->getStatus($feed->id) == 2 || $feed->getStatus($feed->id) == 5 || $feed->getStatus($feed->id) == 6): ?>
    <a href="javascript:void(0);" class="go event_response atndBtn" data-eventId = "<?php echo e($feed->id); ?>" data-status="1"><?php echo e(trans('buttonTxt.attend')); ?></a>
  <?php elseif($feed->getStatus($feed->id) == 3): ?>
  <a href="javascript:void(0);" class="go event_response intBtn" data-eventId = "<?php echo e($feed->id); ?>" data-status="1"><?php echo e(trans('buttonTxt.interested')); ?></a>
  <a href="javascript:void(0);" class="not_go event_response intBtn" data-eventId = "<?php echo e($feed->id); ?>" data-status="6"><?php echo e(trans('buttonTxt.not_interested')); ?></a>
  <?php else: ?>
  <a href="javascript:void(0);" class="go event_response event_btn" data-eventId = "<?php echo e($feed->id); ?>" data-status="1"><?php echo e(trans('buttonTxt.attending')); ?></a>
  <a href="javascript:void(0);" class="not_go event_response event_btn" data-eventId = "<?php echo e($feed->id); ?>" data-status="2"><?php echo e(trans('buttonTxt.not_attending')); ?></a>
  <a href="javascript:void(0);" class="not_go event_response event_btn" data-eventId = "<?php echo e($feed->id); ?>" data-status="3"><?php echo e(trans('buttonTxt.tentative')); ?></a>
  <?php endif; ?>                                
  <?php endif; ?>
...
<?php if( is_member_group(\Auth::guard('user')->user()->id,$feed->group_user_id)>0 ): ?>
...
<a href="<?php echo e(URL::Route('group_details').'/'.$group_id); ?>"> <img src="<?php echo e(asset('uploads/group_images/').'/'.$feed->cover_image); ?>" alt=""/></a>
<?php }else{ ?>
<a href="<?php echo e(URL::Route('group_details').'/'.$group_id); ?>">  <img src="<?php echo e(asset('frontend/images/no-image-event-details.jpg')); ?>"  /></a>
<?php  } ?>
</div>
...
<a href="<?php echo e(URL::Route('group_details').'/'.$group_id); ?>" class="view_all pull-right"> <?php echo __('common.learn_more')?></a>
</div>
</div>
</div>
...
?>         
<?php if( is_member_group(\Auth::guard('user')->user()->id,$feed->group_id)>0): ?>
<?php if(($feed->vimeo_upload == 'No') || ($feed->vimeo_upload == 'Yes' && $feed->is_video_published == 'Yes')): ?>
<div class="timelineBlock">
<div class="time-postedBy">
<div class="image-div">
<?php if($feed->user->profile_photo != NULL && file_exists(public_path('uploads/user_images/profile_photo/thumbnails/' . $feed->user->profile_photo))): ?>
...
<?php if((get_owner_group($feed->group_id) == \Auth::guard('user')->user()->id) || $permission_delete_post==1 || (is_moderator_group(\Auth::guard('user')->user()->id,encrypt($feed->group_id))>0)): ?>
 <div class="nav-func postcrossicon"><span class="delpost"><a href="javascript::void(0);"  style="color:#f29134" alt="<?php echo e($feed->id); ?>" data-toggle="tooltip" em="<?php echo e($feed->group_id); ?>" class="deletepost" title="Delete Post"><i class="fa fa-times" aria-hidden="true"></i></a></span></div>
...
<div class="postedTime-image">
    <?php if($feed->vimeo_upload == 'Yes'): ?>
      <iframe src="https://player.vimeo.com/video/<?php echo e($feed->vimeo_video_id); ?>" width="580" height="420" frameborder="0" mozallowfullscreen webkitallowfullscreen allowfullscreen></iframe>

    <?php endif; ?>

    <?php if($feed->image != NULL && file_exists(public_path('uploads/post_images/' .$feed->image))): ?>
    <a href="<?php echo e(asset('uploads/post_images/'.$feed->image)); ?>" rel="lightbox"> <img src="<?php echo e(asset('timthumb.php')); ?>?src=<?php echo e(asset('uploads/post_images/'.$feed->image)); ?>&w=586&h=586&q=100" alt=""/></a>
    <?php if($feed->text!=''): ?>
    <!-- <p><?php echo nl2br(e($feed->text) ); ?></p> -->
    <p><?php echo nl2br(htmlentities($feed->text)) ?></p>
    <?php endif; ?>        
    <?php else: ?>
    <?php if($feed->text!=''): ?>
    <h2><?php echo nl2br(htmlentities($feed->text)) ?></h2>
    <?php endif; ?>

    <?php endif; ?> 
    <b><?php echo e(trans('home.posted_in')); ?> <a href ="<?php echo e(URL::Route('group_details').'/'.$group_id); ?>"><?php echo e(group_name($feed->group_id)); ?> 
    <!--   - <b>Posted in <a href ="<?php echo e(URL::Route('group_details').'/'.$group_id.'#'.$feed->id); ?>"><?php echo e(group_name($feed->group_id)); ?> -->
    </b></a>
</div>
  <div class="likeComment">
    <div class="row">
    ...
    <button class="face-like face-like-post <?php echo e($cls); ?>"  type="button" id="lkid_<?php echo e($feed->id); ?>" alt="<?php echo e($feed->id); ?>" name="button"><i class="fa fa-thumbs-o-up" aria-hidden="true"></i> <?php echo e(trans('common.like')); ?></button>
...
<div class="comment-other">
    ...
    <img src="<?php echo e(asset('uploads/no_img.png')); ?>" alt="">
    <?php endif; ?>     </div>
    <h2><a href="<?php echo e(URL::Route('user_profile').'/'.($comment->user->ad_username)); ?>"><?php echo e($comment->user->display_name); ?> </a><?php if((get_owner_group($feed->group_id) == \Auth::guard('user')->user()->id) || $permission_delete_comment==1 || (is_moderator_group(\Auth::guard('user')->user()->id,encrypt($feed->group_id))>0)): ?>
...
<!-- for later -->
<?php if($permission_share==1 ||(get_owner_group($feed->group_id) == \Auth::guard('user')->user()->id)): ?> 
    <div class="comment-field">
    <div class="image-div">
    <?php if(\Auth::guard('user')->user()->profile_photo != NULL && file_exists(public_path('uploads/user_images/profile_photo/thumbnails/' . \Auth::guard('user')->user()->profile_photo))): ?>
    <img src="<?php echo e(asset('uploads/user_images/profile_photo/thumbnails/'.\Auth::guard('user')->user()->profile_photo)); ?>" alt=""/>
